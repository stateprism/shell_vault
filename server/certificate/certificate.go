...
func (s *AuthorizedUsers) AddUser(username string, key []byte) bool {
	keyData := make([]byte, len(key))
	keyData = append(keyData, []byte(key)...)
	keyHash := sha512.Sum512(keyData)
	var keyHashStr []byte

	hex.Encode(keyHash[:], keyHashStr)
	if _, ok := s.GetUsers()[string(keyHashStr)]; ok {
		return false
	}
